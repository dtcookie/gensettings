...
type WindowsDetectionCondition struct {
	...
	Property         WindowsServiceProps `json:"property"`                   // Possible Values: `DisplayName`, `ServiceName`, `Path`, `StartupType`, `Manufacturer`
...
func (me *WindowsDetectionCondition) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"property": {
			Type:        schema.TypeString,
			Description: "Possible Values: `DisplayName`, `ServiceName`, `Path`, `StartupType`, `Manufacturer`",
			Required:    true,
		},
		"startup_condition": {
			Type:        schema.TypeString,
