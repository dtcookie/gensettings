...
type WindowsDetectionCondition struct {
	...
	Property         WindowsServiceProps `json:"property"`                   // Possible Values: `Path`, `StartupType`, `Manufacturer`, `DisplayName`, `ServiceName`
...
func (me *WindowsDetectionCondition) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"property": {
			Type:        schema.TypeString,
			Description: "Possible Values: `Path`, `StartupType`, `Manufacturer`, `DisplayName`, `ServiceName`",
			Required:    true,
		},
		"startup_condition": {
			Type:        schema.TypeString,
