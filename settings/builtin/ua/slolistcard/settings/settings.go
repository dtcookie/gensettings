...
type Settings struct {
	...
	Key         string  `json:"key"`                   // Unique key, which is used to map to this SLO list card in the screen layout config
...
func (me *Settings) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"key": {
			Type:        schema.TypeString,
			Description: "Unique key, which is used to map to this SLO list card in the screen layout config",
			Required:    true,
		},
		"scope": {
			Type:        schema.TypeString,
