...
type Settings struct {
	Charts                     ChartDefinitions    `json:"charts,omitempty"`                     // Define charts to display timeseries data in the entities list.
	Columns                    ColumnDefinitions   `json:"columns,omitempty"`                    // Define columns to show additional data in the entities list.
	Conditions                 []string            `json:"conditions,omitempty"`                 // All conditions from the list need to be fulfilled for the List of Entities to be visible
	...
	DisplayIcons               bool                `json:"displayIcons"`                         // When true, entity icon will be displayed in the name column.
	...
	DisplayProblemImpactWidget *bool               `json:"displayProblemImpactWidget,omitempty"` // When true, widget, indicating that some entities are not healthy, will be displayed above the list. Make sure there is visible filtering with health state filter.
	...
	EntitiesLimit              int                 `json:"entitiesLimit"`                        // The limit of entities quantity fetched from the server.
	EntitySelectorTemplates    []string            `json:"entitySelectorTemplates,omitempty"`    // An entity selectors that are used to fetch the selected entities
...
func (me *Settings) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"conditions": {
			Type:        schema.TypeSet,
			Description: "All conditions from the list need to be fulfilled for the List of Entities to be visible",
			Optional:    true, // minobjects == 0
