...
type ColumnDefinition struct {
	...
	Custom     *CustomType    `json:"custom,omitempty"`     // Custom options
	Relation   *RelationType  `json:"relation,omitempty"`   // Relation options
	Type       ColumnType     `json:"type"`                 // Possible Values: `ATTRIBUTE`, `CUSTOM`, `RELATION`
}
...
func (me *ColumnDefinition) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"custom": {
			...
			Optional:    true, // precondition
			Elem:        &schema.Resource{Schema: new(CustomType).Schema()},
			MinItems:    1,
			MaxItems:    1,
		},
		"relation": {
		...
		"type": {
			...
			Description: "Possible Values: `ATTRIBUTE`, `CUSTOM`, `RELATION`",
			Required:    true,
		},
	}
}
...
func (me *ColumnDefinition) HandlePreconditions() error {
	...
	if me.Custom == nil && (string(me.Type) == "CUSTOM") {
		return fmt.Errorf("'custom' must be specified if 'type' is set to '%v'", me.Type)
	}
	if me.Custom != nil && (string(me.Type) != "CUSTOM") {
		return fmt.Errorf("'custom' must not be specified if 'type' is set to '%v'", me.Type)
	}
