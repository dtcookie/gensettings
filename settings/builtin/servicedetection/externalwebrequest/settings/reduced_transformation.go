...
type ReducedTransformation struct {
	...
	TransformationType ContextRootTransformationType `json:"transformationType"` // Possible Values: `REMOVE_IPS`, `BEFORE`, `REPLACE_BETWEEN`, `REMOVE_NUMBERS`, `REMOVE_CREDIT_CARDS`, `REMOVE_IBANS`
}
...
func (me *ReducedTransformation) Schema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"transformation_type": {
			Type:        schema.TypeString,
			Description: "Possible Values: `REMOVE_IPS`, `BEFORE`, `REPLACE_BETWEEN`, `REMOVE_NUMBERS`, `REMOVE_CREDIT_CARDS`, `REMOVE_IBANS`",
			Required:    true,
		},
	}
}
